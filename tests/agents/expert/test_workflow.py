...
class TestTaskWorkflow(unittest.TestCase):
    ...
    def test_create_workflow_loop_output_node(self) -> None:
        ...
        self.assertEqual(new_state["output_state"]["test_node"].output_type, "LOOP_MESSAGE_TRUE")
    ...
    @patch('fluctlight.agents.expert.workflow.chat_completion')
    def test_create_conditional_edge_chain(self, mock_chat_completion) -> None:
        mock_response = MagicMock()
        mock_response.choices[0].message.parsed = ConditionalOutput(is_match_success_criteria=True)
